...
uniform float u_seaweed_depth;
uniform float u_seaweed_depth2;
uniform vec2 seaweed_coord;
...
varying vec2 v_seaweed_texcoord;
varying vec2 v_seaweed_texcoord2;
...
void main (void) {
    ...
    vec3 filterNormal=vec3(1,0.8,0.5);
    v_mask=vec3(exp(-diw*filterNormal.x),exp(-diw*filterNormal.y),exp(-diw*filterNormal.z));
}
""")
...
uniform sampler2D u_seaweed_texture;
uniform sampler2D u_seaweed_texture2;
...
void main() {
    ...
    vec3 background;
    vec3 seaweed_color=texture2D(u_seaweed_texture, v_seaweed_texcoord).rgb;
    vec3 seaweed_color2=texture2D(u_seaweed_texture2, v_seaweed_texcoord2).rgb;
    vec3 normal=normalize(v_normal);
    ...
    vec3 image_color;
    if(!inWater && (seaweed_color[2]>0 || seaweed_color[1]>0 || seaweed_color[0]>0) && (u_seaweed_depth < u_bed_depth && u_seaweed_depth > 0) 
        && (u_seaweed_depth < u_seaweed_depth2 || !(seaweed_color2[2]>0 || seaweed_color2[1]>0 || seaweed_color2[0]>0))) {
        image_color=u_depth_mult*ambient_water*(1-v_mask)+u_seaweed_mult*seaweed_color*(v_mask);
    }
    else if (!inWater && (seaweed_color2[2]>0 || seaweed_color2[1]>0 || seaweed_color2[0]>0) && (u_seaweed_depth2 < u_bed_depth && u_seaweed_depth2 > 0)){
        image_color=u_depth_mult*ambient_water*(1-v_mask)+u_seaweed_mult*seaweed_color2*(v_mask);
    }
    else {
...
class Canvas(app.Canvas):
    ...
    def __init__(self, surface, sky="fluffy_clouds.png", bed="seabed.png"):
        ...
        self.seaweed2 = io.read_png("seaweed.png")
        # create GL context
        ...
        self.program['u_seaweed_texture'] = gloo.Texture2D(self.seaweed)
        self.program['u_seaweed_texture2'] = gloo.Texture2D(self.seaweed2)
        ...
        self.program["seaweed_coord"] = [1, 1]
        self.program["seaweed_diff"] = 0
        self.program["seaweed_coord2"] = [-0.5, -0.5]
        self.program["seaweed_diff2"] = np.pi / 2
    ...
    def apply_flags(self):
        ...
        self.program["u_seaweed_mult"] = 1 if self.seaweed_flag else 0
    ...
    def on_key_press(self, event):
        if event.key == 'Escape':
            ...
            self.seaweed_flag = not self.seaweed_flag
            print("Show refracted image of seaweed:", self.seaweed_flag)
            self.apply_flags()
        elif event.key == 'q':
            self.program["seaweed_coord"] = [0.2, -0.2]
        elif event.key == 'w':
            self.program["seaweed_coord"] = [0.2, 0.2]
        elif event.key == 'e':
            self.program["seaweed_coord"] = [-0.2, 0.2]
    ...
    def on_mouse_wheel(self, event):
        if self.shift:
            ...
            self.program["u_seaweed_depth"] -= event.delta[1] / 10
        elif self.x:
            self.program["u_seaweed_depth2"] -= event.delta[1] / 10
        else:
            if ((self.program["u_eye_height"] < 15 or event.delta[1] > 0) and (
                    self.program["u_eye_height"] >= 0 or event.delta[1] < 0)):
